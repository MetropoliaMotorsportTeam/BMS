...
void operation_main(void)
{
  ...
  while (1)
  {
    ...
    if (status_data.accu_state == INIT)
    { // estimate battery capacity during non active HV
...
int AMS_OK(status_data_t* status_data, limit_t* limit)
{
  if (status_data->min_voltage > limit->min_voltage &&
      status_data->max_voltage < limit->max_voltage)
  {
    ...
    if (status_data->min_temp > limit->min_temp && status_data->max_temp < limit->max_temp)
    {
      ...
      else if (status_data->air_s &&
               (HAL_GetTick() - status_data->last_ivt_tick) < IVT_LOSS_GRACE_MS)
      {
  ...
  status_data->accu_state = ACCU_ERROR;
  open_AIR();
  return 1;
}
...
void precharge_compare(void)
{
  ...
  {
    if ((percentage >= 95) && (check_voltage_match() == true) &&
        status_data.IVT_U1_f > limits.precharge_min_start_voltage)
    {
