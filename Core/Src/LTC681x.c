...
void wrcfg(uint8_t total_ic, uint8_t config[][6])
{
  ...
  for (uint8_t current_ic = total_ic; current_ic > 0; current_ic--)
  {
         ...
         current_byte++) /* executes for each of the 6 bytes in the CFGR register */
    {
      ...
      cmd[cmd_index] =
          config[current_ic - 1][current_byte]; /* adding the config data to the array to be sent */
      cmd_index = cmd_index + 1;
    }
    cfg_pec = (uint16_t)pec15_calc(BYTES_IN_REG,
...
void wrcfgb(uint8_t total_ic, uint8_t config[][6])
{
  const uint8_t BYTES_IN_REG = 6;
  const uint8_t CMD_LEN = 4 + (8 * total_ic);
  uint16_t cfg_pec;
...
int8_t rdcfgb(uint8_t total_ic, uint8_t r_config[][8])
{
  const uint8_t BYTES_IN_REG = 8;
...
)
{
  ...
  if (reg == 0)
  {
    for (uint8_t cell_reg = 1; cell_reg < 5;
         cell_reg++) // executes once for each of the LTC6811 cell voltage registers (A-D, 4
                     // registers, 12 cells)
    {
      data_counter = 0;
      ...
      for (uint8_t current_ic = 0; current_ic < total_ic;
           current_ic++) // executes for every LTC6804 in the daisy chain
      {
             ...
             current_cell++) // This loop parses the read back data into cell voltages, it
        {
          ...
          parsed_cell = cell_data[data_counter] +
                        (cell_data[data_counter + 1]
                         << 8); // Each cell code is received as two bytes and is combined to
          // create the parsed cell voltage code
          // cell_codes[current_ic][current_cell  + ((cell_reg - 1) * CELL_IN_REG)] = parsed_cell;
          cell_codes[current_ic][current_cell + ((cell_reg - 1) * CELL_IN_REG)].voltage =
              parsed_cell;
          data_counter =
              data_counter + 2; // Because cell voltage codes are two bytes the data counter
        ...
        data_counter =
            data_counter + 2; // Because the transmitted PEC code is 2 bytes long the data_counter
  ...
  else
  {
         ...
         current_ic++) // executes for every LTC6804 in the daisy chain
    {
           ...
           current_cell++) // This loop parses the read back data into cell voltages, it
      {
        ...
        // cell_codes[current_ic][current_cell + ((reg - 1) * CELL_IN_REG)] = 0x0000FFFF &
        // parsed_cell;
        cell_codes[current_ic][current_cell + ((reg - 1) * CELL_IN_REG)].voltage =
            0x0000FFFF & parsed_cell;
        data_counter = data_counter + 2; // Because cell voltage codes are two bytes the data
                                         // counter
        // must increment by two for each parsed cell code
      }
      received_pec =
...
)
{
  ...
  if (reg == 0)
  {
    for (uint8_t gpio_reg = 1; gpio_reg < 3;
         gpio_reg++) // executes once for each of the LTC6811 aux voltage registers (A-B only, 5
                     // GPIOs + Vref2)
    {
      data_counter = 0;
      rdaux_reg(gpio_reg, total_ic,
           ...
           current_ic++) // executes for every LTC6804 in the daisy chain
      {
        ...
        for (uint8_t current_gpio = 0; current_gpio < GPIO_IN_REG;
             current_gpio++) // This loop parses the read back data into GPIO voltages, it
        {
          // loops once for each of the 3 gpio voltage codes in the register

          parsed_aux = data[data_counter] +
                       (data[data_counter + 1]
                        << 8); // Each gpio codes is received as two bytes and is combined to
          ...
          data_counter =
              data_counter + 2; // Because gpio voltage codes are two bytes the data counter
  ...
  else
  {
         ...
         current_ic++) // executes for every LTC6804 in the daisy chain
    {
      ...
      for (int current_gpio = 0; current_gpio < GPIO_IN_REG;
           current_gpio++) // This loop parses the read back data. Loops
      {
        ...
        data_counter = data_counter + 2; // Because gpio voltage codes are two bytes the data
                                         // counter
        // must increment by two for each parsed gpio voltage code
      }
      received_pec =
...
int8_t
rdaux1(uint8_t reg,                      // Determines which GPIO voltage register is read back.
       uint8_t total_ic,                 // the number of ICs in the system
       temp_data_t aux_codes[][GPIO_NUM] // A two dimensional array of the gpio voltage codes.
)
{
  ...
  for (uint8_t gpio_reg = 4; gpio_reg > 0; gpio_reg--)
...
)
{
  ...
  for (uint8_t current_cell = 0; current_cell < CELL_IN_REG;
       current_cell++) // This loop parses the read back data into the register codes, it
  ...
  received_pec =
      (cell_data[data_counter] << 8) |
      cell_data[data_counter + 1]; // The received PEC for the current_ic is transmitted as the 7th
                                   // and 8th after the 6 cell voltage data bytes
  data_pec = pec15_calc(BYT_IN_REG, &cell_data[(current_ic)*NUM_RX_BYT]);
...
int8_t rdstata(uint8_t total_ic,     // Number of ICs in the system
               uint8_t r_config[][8] // A two dimensional array that the function stores the read
                                     // configuration data.
)
{
  const uint8_t BYTES_IN_REG = 8;
  ...
  for (uint8_t current_ic = 0; current_ic < total_ic;
       current_ic++) // executes for each LTC6804 in the daisy chain and packs the data
  {
    // into the r_config array as well as check the received Config data
    // for any bit errors
